 ...
 * @author Le Phuoc Duy
 */
@WebServlet(name = "DeleteRequestController", urlPatterns = {"/DeleteRequestController"})
public class DeleteRequestController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (daoR.deleteRequest(requestID)) {
                for (RequestDetailDTO x : dtoD) {
                    daoP.updateQuanlity(x.getQuantity(), x.getProductID());
                }
            }    
            List<RequestDTO> result = daoR.requestListHistory(email);
            request.setAttribute("REQUESTHISTORYLIST", result);
        } catch (Exception e) {
            log("ERROR at DeleteRequestController: " + e.getMessage());
        } finally {
